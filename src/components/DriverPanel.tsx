...
import { Trash2, Plus, User, Phone, UserX, PaintBucket, UserCheck, Pencil, Save, RefreshCcw, ChevronLeft, ChevronRight } from 'lucide-react';
...
const DriverPanel: React.FC<{ onToggleCollapse?: (isOpen: boolean) => void }> = ({ onToggleCollapse }) => {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="p-4 header-gradient rounded-t-lg flex justify-between items-center">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            onClick={togglePanel}
            className="h-8 w-8"
            title={isOpen ? "Hide drivers panel" : "Show drivers panel"}
          >
            {isOpen ? (
      ...
      <Collapsible open={isOpen} onOpenChange={setIsOpen} className="flex-grow flex flex-col">
        <CollapsibleContent className="flex-grow flex flex-col data-[state=closed]:hidden">
      ...
      {!isOpen && (
        <div className="p-4 flex justify-center border-t">
          <Button 
            ...
            onClick={togglePanel}
          >
            <ChevronRight className="h-4 w-4 mr-2" /> Show Drivers
          </Button>
        </div>
      )}
    </div>
